...
describe("Test DAO", function () {
  ...
  it("Check that user can't vote if haven't deposited any tokens", async () => {
    ...
    await expect(DAO.connect(user1).vote(firstProposalId)).revertedWith("No suffrage");
  });
  ...
  it("Check that user can't vote after deadline", async () => {
    ...
    await expect(DAO.connect(user1).vote(firstProposalId)).revertedWith("Proposal voting ended");
  });

  it("Check voting behaviour", async () => {
    await token.connect(user1).approve(DAO.address, testUser1Amount);
    await DAO.connect(user1).deposit(testUser1Amount);
    await DAO.connect(chairPerson).addProposal(testCallData, testRecepient, testDescription);
    const votingTransaction = await DAO.connect(user1).vote(firstProposalId);
    ...
    expect(totalVotes).to.equal(testUser1Amount);
  });
  ...
  it("Check that user can't vote twice without new depposit", async () => {
    ...
    await token.connect(user1).approve(DAO.address, testUser1Amount);
    await DAO.connect(user1).deposit(testUser1Amount);
    await DAO.connect(user1).vote(firstProposalId);

    await expect(DAO.connect(user1).vote(firstProposalId)).to.be.revertedWith("No suffrage");;
  });
  ...
  it("Check that user votes are combined", async () => {
    ...
    const votingTransaction = await DAO.connect(user2).vote(firstProposalId);
    const rc = await votingTransaction.wait();
    const votedEvent = rc.events.find((e: { event: string }) => e.event == 'Voted');

    const [[proposalId, totalVotes, deadline, recipient, isActive, callData, description]] = votedEvent.args;
    expect(totalVotes).to.equal(testUser1Amount.add(testUser2Amount));
  });
  ...
  it("Check that user can vote in multiple votings with the same tokens", async () => {
    ...
    const votingTransaction1 = await DAO.connect(user1).vote(firstProposalId);
    const rc1 = await votingTransaction1.wait();
    const votedEvent1 = rc1.events.find((e: { event: string }) => e.event == 'Voted');

    const [[proposalId1, totalVotes1,]] = votedEvent1.args;
    expect(totalVotes1).to.equal(testUser1Amount);

    const votingTransaction2 = await DAO.connect(user1).vote(secondProposalId);
    const rc2 = await votingTransaction2.wait();
    const votedEvent2 = rc2.events.find((e: { event: string }) => e.event == 'Voted');

    const [[proposalId2, totalVotes2,]] = votedEvent2.args;
    expect(totalVotes2).to.equal(testUser1Amount);
  });
  ...
  it("Check proposal finishing, case of enough quorum", async () => {
    ...
    await token.connect(user2).approve(DAO.address, testUser2Amount);
    await DAO.connect(user2).deposit(testUser2Amount);
    await DAO.connect(user2).vote(firstProposalId);
  ...
  it("Check that user can't vote in finished proposal votings", async () => {
    ...
    await expect(DAO.connect(user1).vote(firstProposalId)).to.be.revertedWith("Proposal voting is not active");
  });
  ...
  it("Check that user can't withdraw if one voting is ended, but another is still active", async () => {
    ...
    await DAO.connect(user1).deposit(testUser1Amount);
    await DAO.connect(user1).vote(firstProposalId);
    await DAO.connect(user1).vote(secondProposalId);
